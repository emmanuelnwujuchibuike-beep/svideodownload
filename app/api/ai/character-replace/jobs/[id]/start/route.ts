...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!pathBelongsTo(meta.video.path, ownerId, job.id) || !pathBelongsTo(meta.character.path, ownerId, job.id)) {
      console.error("[cr/start] a path failed ownership", { jobId: job.id, subject: subject.key });
      return fail("INTERNAL_ERROR");
    }

    const [settings, entitlement, videoObject, characterObject] = await Promise.all([
      getLandingSettings(),
      getAiEntitlement(subject, feature),
      statSourceObject(meta.video.path),
      statSourceObject(meta.character.path),
    ]);
    ...
    if (!characterObject || characterObject.size <= 0) return notYet("photo");
    if (videoObject.size > feature.maxBytes) return fail("FILE_TOO_LARGE");
    if (characterObject.size > AI_IMAGE_MAX_BYTES) return fail("FILE_TOO_LARGE");

    /* ── B · calculate ───────────────────────────────────────────────────── */
    const money = { currency: settings.frenzAiCurrency, symbol: aiCurrencySymbol(settings.frenzAiCurrency) };
    const verdict = verifyStartQuote(body, config, money);
    if (!verdict.ok) {
