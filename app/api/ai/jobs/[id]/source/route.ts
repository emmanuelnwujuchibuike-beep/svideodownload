...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  */
  ...
  try {
    ...
    /*
      Part 7 §5: for a Character Replace job the "original" the member compares
      against is the PREPARED cut — the exact frames and length the model
    ...
    */
    ...
    const signed = await signSourceViewUrl(sourcePath);
    return NextResponse.json(
