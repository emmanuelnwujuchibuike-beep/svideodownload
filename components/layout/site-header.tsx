      ...
      {open && (
        <div className="max-h-[calc(100vh-4rem)] overflow-y-auto border-t border-border/40 bg-background/95 backdrop-blur-xl md:hidden">
          <nav className="container flex flex-col gap-1 py-4">
            ...
            {/* All downloader pages — premium brand grid */}
            <p className="mt-3 px-3 text-[11px] font-semibold uppercase tracking-[0.18em] text-muted-foreground/70">
              Download from
            </p>
              ...
              {DOWNLOADERS.map((d) => {
                ...
                return (
                  <Link
                    ...
                    href={`/${d.slug}`}
                    onClick={() => setOpen(false)}
                    className="group flex items-center gap-2.5 rounded-xl border border-border bg-card/70 p-2.5 shadow-soft transition active:scale-[0.98] hover:border-foreground/20"
                  >
                    <span
