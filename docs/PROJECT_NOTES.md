...
### The balance in dollars, a live naira rate at checkout (`ad39a64`, migration 0159)
- **0159** flipped the stored wallet to USD in one guarded transaction: the one non-zero balance (₦51,126.26 → $38.28 at ₦1,335.694724/$1, open.er-api.com that day) with an adjustment-out (NGN) and an adjustment-in (USD) ledger pair; every price kobo → cents at the same rate (720p ₦150/s → 11¢/s, Skin + Face 7/11/15¢, lip sync 11/19¢, TTS ₦500 → 37¢, min charge 1¢); recharge $1–$500 with $5/$10/$25/$50/$100; `checkoutCurrency: NGN`; `localMinorUnitsPerUsd: 133569` as the manual fallback; pricingVersion 4 → 5 with history. Old ledger rows keep `currency: NGN` and the balance page prints each line with its own symbol.
- **The live rate** (`lib/ai/character-replace/fx-rate.ts` pure + `fx-rate-server.ts`): memory (1 h) → open.er-api.com → jsDelivr `@fawazahmed0/currency-api` → the last-good rate in `settings.fx_rates` (≤ 7 days) → the manual rate → 503. `recharge.fxMarkupPercent` (0–50) sits on top of a MARKET rate only. The initializer and the balance route share it, the pin makes the credit exact. Admin → Recharge shows the live readout (`GET /api/admin/ai/character-replace/fx`).
- **Balance page:** tap the figure/eye to hide (localStorage `frenz-ai:balance-hidden`, shared with the workspace card); tap a statement line → `features/ai/statement-detail-sheet.tsx` with the transaction id, a recharge's Paystack reference, the video id, the priced facts, Copy details, Get help. The ledger reader now returns `reference` (recharge only) and `details` from the snapshot.
...
**Part 7 — the result experience.** `/studio/ai/character-replace/result/[id]` is the one door
(push deep link, history tile, "Video Ready"): server ownership check (`getOwnJob` → `notFound()`,
sign‑in redirect with `next=`), noindex; it draws the right screen for ANY status. Video Ready:
player with poster/seek/volume/fullscreen/speed, Original ↔ result comparison from the PREPARED cut
(signed 600 s), facts from the output's own probe (fps only when measured), Download through the one
download manager, Save (`POST /api/ai/jobs/[id]/save` — moves expiry only, never copies or
charges), Share through Web Share with the file (no public link is ever minted), Video details with
only the fields the job used, "Created with Frenz AI · AI‑generated video", Create Another / Use
same photo, Delete with confirm. Migration **0157** adds status `deleted` (terminal; the store hides
it from every list; files + paths cleared, **ledger untouched**). Retention is configuration
(`retention.resultHours` 72, `savedResultDays` 30). History: Character Replace tiles open the result
route in every status; "Refunded ✓" only when the ledger confirms it. Owner asks the same day: red
direction boxes on both input steps, a full‑body icon for Full Character, "View tutorial example"
sheet with animated guides, the AI button on `/history` only.

- ⛔ **A `"use client"` VALUE import from `lib/landing/settings.ts` drags the whole Character
  Replace config into the eager `/admin` chunk** (settings.ts builds its defaults from the CR
  normaliser at load). /admin went 371 kB > 368 ceiling; the fix is the pure
  `lib/landing/bounds.ts` (settings.ts re‑exports) and the guard `lib/perf/client-imports.test.ts`.

**Production E2E (throwaway member, 3 s fixtures, `scripts/_e2e-character-replace.tmp.mjs`,
untracked):** Face Only end‑to‑end ✓ (≈50 s, settled once, webhook, push, save, IDOR 404s, delete);
**voice (MiniMax TTS) → Face Only → Studio Lip Sync completed end‑to‑end ✓** (≈205 s, one charge).
The first voice run FAILED and exposed a real bug: Replicate throttled the replace‑stage submit
(429 — a low‑credit account gets one prediction a minute); the worker correctly left the job for the
sweep but answered `ok:false`, which the dispatcher read as "the worker declined" → the webhook
ended the job with FINALIZER_UNAVAILABLE and refunded a job that needed sixty more seconds.

**Part 8 — hardening (`b4beb46`, migration 0158).**
- **Retry is a hand‑off, not a decline**: worker services say `retry: true` on their retry
  branches; `finalize-dispatch.ts` treats `ok:false + retry:true` as dispatched.
- **The reconciler is stage‑aware**: it acts only on the CURRENT stage's in‑flight prediction (a
  finished voice stage's id is never "reconciled" into a finalization of a speech file); a lost
  callback for an intermediate stage dispatches the ADVANCE; `pipeline.advance()` stamps
  `stage_started_at`; a never‑created stage is re‑submitted after 70 s.
- **Switches** (`config.ops`): `processingEnabled` (pause new starts, running jobs finish),
  `maintenanceMode` + message (creates/starts refused, results/history stay; the workspace shows the
  operator's sentence), circuit‑breaker thresholds. **Limits** (`config.limits`): active per member
  (tightens the plan cap), active platform‑wide (default 25 → `CR_BUSY`), per member per day.
  `localMinorUnitsPerUsd` powers margin warnings in the admin form.
- **Race‑safe /start**: `claim_ai_job_start()` moves queued → acquiring INSIDE one advisory lock
  with the three counts. Order: switches → breaker → claim → reserve → hand‑off; a failed
  reservation reverts the claim (`revertJobStartClaim`). ⛔ The old reserve‑then‑claim order could
  not refuse on a limit: `reserve_product_charge` is idempotent per job, so a refunded reservation
  would let the next Start run the job for free. Falls back to the plain CAS until 0158 is applied.
- **Circuit breaker** (`lib/ai/character-replace/circuit.ts`, table `ai_provider_health`): every
  provider submit runs under `withCircuit` — refused with PROVIDER_UNAVAILABLE (transient; paid
  jobs wait) while open, scored after; /start refuses NEW jobs for a paused model (nothing
  charged); opens after N failures in a window, half‑open probe after the cooldown; admin alert
  once per opening; Admin → AI → "Providers & changes" shows state + "Close now" (reason recorded).
- **Audit log**: every Character Replace settings save writes the changed keys, before/after,
  admin id and reason to the platform's `config_audit_log` (surface `character_replace`).
- New codes `CR_ACTIVE_LIMIT` / `CR_DAILY_LIMIT` / `CR_BUSY` / `CR_MAINTENANCE` (each "nothing was
  charged"). `/api/ai/jobs/[id]/result` answers 404 for a deleted job. Retention sweeps queued
  projects never started within 24 h. Index `ai_jobs_active_feature_idx`.
- Owner fixes: Face Only's Continue stayed grey (the readiness gate read the Full Character quality
  list — `qualityOffered()` reads the mode's tiers); "Start Creating"/tool cards/crumb prefetch by
  default and carry `LinkPendingStripe` (Next `useLinkStatus`, a top stripe after 120 ms when the
  route was not cached); the nav warms `/history` and `/studio/ai/history` after mount.
- Known: deleting an auth user cascades `ai_jobs`, `ai_job_events` AND `ai_product_ledger` (a
  purged account takes its financial rows with it — by design of the purge, noted for the owner).
  The result PAGE streams (a `loading.tsx` above it) so a stranger gets HTTP 200 with the not‑found
  UI and no job data; every API answers 404. Full Character has no operator cost figure yet, so its
  margin is not checked.
