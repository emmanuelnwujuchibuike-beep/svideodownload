  ...
  */
  ...
  return (
    <section className="mt-6 rounded-3xl border border-border bg-card px-3 py-6 sm:px-6 shadow-card">
      ...
      {/*
        🔴 ONE TAB PER NETWORK (owner, 2026-09-01: "the ad placement in admin is
        scatered, arrange all ad network config to be grouped in a top nav, each
        network, each page").

        The panel had grown by accretion: ExoClick's sticky and bottom-nav fields,
        then HilltopAds, then more ExoClick fields BELOW those, then AdSense,
        Monetag and the verification tags. An operator configuring one network was
        scrolling past three others to do it, and the two halves of ExoClick were
        separated by the whole of HilltopAds.

        `AdminSubsections` is the component the admin already uses for exactly this
        (features/admin/section-tabs.tsx) rather than a second tab implementation.
        It HIDES the inactive groups instead of unmounting them, which is required
        here and not incidental: these panels are FORMS, and unmounting one would
        discard whatever the operator had typed into it the moment they looked at
        another network.

        Nothing inside the groups changed. Every block is the same JSX, moved.
      */}
      <AdminSubsections
        groups={[
          {
          ...
          */}
          ...
          <div className="mt-2.5 rounded-2xl border border-border/70 bg-secondary/20 p-3.5">
            ...
            <HilltopField
              ...
              onSave={(v) => void persist({ ...state, hilltopVideoSliderSnippet: v })}
            />
          </div>
          ...
          <div className="mt-2.5 rounded-2xl border border-border/70 bg-secondary/20 p-3.5">
            ...
            <p className="mt-0.5 mb-2 text-xs leading-relaxed text-muted-foreground">
              Paste the <strong>VAST 3.0 tag URL</strong> from Hilltop — a plain
              <code className="mx-1 rounded bg-background px-1 py-0.5 font-mono text-[10px]">https://…</code>
              address, not a script block. It is one URL rather than one per position: a
              VAST tag is called by the player and is not tied to a placement the way a
              banner is.
            </p>
            <input
              type="url"
              value={state.hilltopVastUrl ?? ""}
              disabled={busy}
              onChange={(e) => setText("hilltopVastUrl", e.target.value)}
              placeholder="https://…/d/…"
              className="h-10 w-full rounded-xl bg-background px-3 font-mono text-xs outline-none ring-1 ring-inset ring-border focus:ring-2 focus:ring-primary"
            />
            <div className="mt-2 flex flex-wrap items-center gap-2">
              ...
              {parseHilltopVastUrl(state.hilltopVastUrl ?? "") ? (
                <span className="text-[11px] font-medium text-green-600 dark:text-green-400">
                  Read tag {new URL(state.hilltopVastUrl!.trim()).hostname}
                </span>
              ) : state.hilltopVastUrl?.trim() ? (
                <span className="flex items-center gap-1.5 text-[11px] font-medium text-amber-600 dark:text-amber-400">
                  <AlertTriangle className="h-3.5 w-3.5" /> Not a usable https URL.
                </span>
              ) : (
          ...
          */}
          ...
          {
            id: "moments",
            label: "Interstitials and rewards",
            content: (
              <>
          {/*
      ...
      {/* Outside the tabs on purpose: it saves the text fields in SEVERAL groups
          (AdSense, the Google tag, verification), so it has to be reachable from
 ...
 */
/**
 * The HilltopAds snippet field.
 *
 * 🔴 ITS OWN COMPONENT BECAUSE IT VALIDATES A DIFFERENT THING (owner,
 * 2026-09-01: "the hiltop slot is not working, is showing could not read a
 * zone").
 *
 * The Hilltop fields were using `SnippetField`, which is ExoClick's: it parses
 * with `parseExoClickSticky` and looks for an `<ins class="eas…"
 * data-zoneid="…">`. A Hilltop snippet has neither — it is a script loader with
 * a `src` — so a PERFECTLY VALID tag was reported as "Could not read a zone id
 * and an eas… class — it will not show". The readout was wrong, not the tag,
 * and it said the one thing guaranteed to stop an operator saving.
 *
 * ⚠️ It also SAVES THE VALUE IT IS SHOWING. `SnippetField`'s callers do
 * `onSave={() => void persist(state)}`, which persists whatever `state` the
 * surrounding render closed over. That is fine when a re-render has happened
 * in between and a trap when one has not. Here the committed text is handed
 * back to the caller, so what is saved is exactly what is in the box.
 */
function HilltopField({
  value,
  busy,
  onChange,
  onSave,
  ...
  const tag = parseHilltopTag(value);
  return (
    <>
      <textarea
        value={value}
      ...
      <div className="mt-2 flex flex-wrap items-center gap-2">
        ...
        {tag ? (
          <span className="text-[11px] font-medium text-green-600 dark:text-green-400">
            Read loader {new URL(tag.src).hostname}
          </span>
        ) : value.trim() ? (
          <span className="flex items-center gap-1.5 text-[11px] font-medium text-amber-600 dark:text-amber-400">
            <AlertTriangle className="h-3.5 w-3.5" /> No https script URL found — paste the whole
            &lt;script&gt; block from Hilltop.
          </span>
        ) : (
          <span className="text-[11px] text-muted-foreground">Off — nothing pasted.</span>
        )}
