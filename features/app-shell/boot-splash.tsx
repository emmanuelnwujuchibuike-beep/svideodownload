 ...
 *
 * CURRENT RULE (owner, 2026-08-02, revised again: "remove the F loader entirely if
 * it keeps coming to the browser … i don't want the F loader to show on any
 * switching"): the boot F loader is INSTALLED-PWA ONLY. A plain browser tab NEVER
 * shows it — not on a cold entry, a reload, a mode switch, or a sign-out. In the
 * installed PWA it still shows on a genuine cold launch and a fresh sign-in. (The
 * brief sign-in loader on the /login → app hop is a SEPARATE overlay — FLoaderOverlay
 * in auth-panel — not this boot splash.) The whole browser show-path is gated on
 * `standalone`, so nothing in a browser can ever reach it.
 *
...
// 2026-08-16: default mode reverted to SYSTEM (was hardcoded "light" here
// since 2026-07-16 — see the matching, unabridged note on `readInitial` in
// lib/theme/theme-mode-client.ts for why that was safe to revert: the actual
// anti-flash mechanism is `syncNextThemes()` a few lines below, which runs
// identically regardless of what the default resolves to, so "system" was
// never the cause of the original bug). `mode()` now also PERSISTS "system"
// into MODE the first time it finds the key empty — the owner asked a first-
// time visitor's choice be "saved on local storage", not merely assumed each
// load — mirroring the same write `readInitial` performs on the React side.
const THEME_JS = `(function(){var CACHE='frenz-resolved-theme';var MODE='frenz-theme-mode';var mq=window.matchMedia('(prefers-color-scheme: dark)');function mode(){try{var v=localStorage.getItem(MODE);if(v)return v;localStorage.setItem(MODE,'system');return 'system'}catch(e){return 'system'}}function cached(){try{return localStorage.getItem(CACHE)}catch(e){return null}}function remember(v){try{localStorage.setItem(CACHE,v)}catch(e){}}function syncNextThemes(v){try{localStorage.setItem('theme',v)}catch(e){}}function set(dark){document.documentElement.classList.toggle('dark',dark)}function resolveSystem(){var c=cached();if(c)return c;return mq.matches?'dark':'light'}function apply(v){set(v==='dark');remember(v);syncNextThemes(v)}function boot(recheck){var m=mode();if(m==='light'||m==='dark'){apply(m);return}var resolved=resolveSystem();apply(resolved);if(recheck){setTimeout(function(){if(mode()!=='system')return;var live=mq.matches?'dark':'light';if(live!==resolved)apply(live)},60)}}boot(true);function onSystemChange(){if(mode()!=='system')return;apply(mq.matches?'dark':'light')}try{if(mq.addEventListener)mq.addEventListener('change',onSystemChange);else if(mq.addListener)mq.addListener(onSystemChange)}catch(e){}window.addEventListener('pageshow',function(){boot(false)})})();`;
