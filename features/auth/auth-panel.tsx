...
/**
 * Marks the instant right before the browser navigates into the app after a
 * real sign-in — read once and cleared by `SessionSplash`, which turns it
 * into the TikTok/Twitter-style full-screen welcome. Short-lived (60s) as a
 * safety net in case it's ever read somewhere that doesn't clear it.
 */
function markJustSignedIn() {
  try {
    document.cookie = "frenz_just_signed_in=1; path=/; max-age=60; SameSite=Lax";
  } catch {
    /* cookies blocked — no splash, not worth failing sign-in over */
  }
}
