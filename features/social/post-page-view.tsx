...
// Aliased — callers export the reserved route-segment config
// `export const dynamic = "force-dynamic"`, which would otherwise collide
// with next/dynamic's default export name.
...
import { getPostMediaItems, recordPostView, relatedPosts, type PublicPost } from "@/lib/social/posts";
...
/**
 * Metadata for a resolved post, keyed to wherever it's actually mounted.
 * `canonicalPath` is the caller's decision, not this file's — app/p/[id]
 * passes `/p/${id}` (or redirects before ever calling this, once a slug
 * exists), the SEO slug route passes its own `postHref(post)` path — so
...
/**
 * The full post page — media, engagement, creator card, poll, comments,
 * related posts, structured data. Shared by app/p/[id]/page.tsx (the
 * permanent id-based URL, source of truth when a post has no slug yet) and
 * app/[category]/[year]/[month]/[slug]/page.tsx (the SEO canonical once one
 * exists) so both render identically and only ever differ in which URL they
...
export async function PostPageView({
  ...
  */
  ...
  /*
    NewsArticle — ONLY for category="news" posts carrying a genuinely
    substantial description. A one-line caption on a reposted clip is a
    VideoObject/ImageObject, not a news article; marking every "news"-tagged
    post as NewsArticle would be exactly the overclaim Google's own structured
    data guidelines (and this project's no-fabrication rule) warn against. All
    fields below are real, already-loaded values — no invented author, no
    invented dateModified (there's no updated_at column, so it's honestly the
    same as datePublished rather than a fabricated "last updated" date).
  */
  const isSubstantialNews = post.category === "news" && (post.description?.length ?? 0) >= 150;
  const newsLd = isSubstantialNews
    ? {
        "@type": "NewsArticle",
        headline: post.title,
        description: post.description,
        ...
        author: { "@type": "Person", name: post.source_author || post.publisher.displayName },
        publisher: {
          "@type": "Organization",
          name: "Frenzsave",
  ...
  const ld = {
    ...
    "@graph": newsLd ? [mediaLd, newsLd] : [mediaLd],
  };
        ...
        <div className="mt-5 flex flex-wrap items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-muted-foreground">
              ...
              {post.category ? (
                <Link href={`/${post.category}`} className="rounded-full bg-secondary px-2 py-0.5 text-xs font-medium text-foreground transition hover:bg-secondary/70">
                  #{post.category}
                </Link>
              ) : null}
            </div>
