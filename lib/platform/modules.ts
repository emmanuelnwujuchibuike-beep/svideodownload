...
export const MODULES: PlatformModule[] = [
  {
     ...
     * minus `youtube`, unpromoted 2026-08-25 after two AdSense "low value
     * content" rejections — see config/seoPages.ts's removal note; the
     * backend still supports youtube.com links, it's just not advertised).
     * It is pinned by a test against SHOWCASE_PLATFORMS.length so it cannot
     * drift again in either direction, and it matters more than a normal
     * copy error because this tagline is republished verbatim into
     * /llms.txt, which exists to be believed by systems that cannot check
