...
describe("normalizeOrder", () => {
  ...
  it("preserves a full valid saved order verbatim", () => {
    const saved = ["continue_watching", "stories", "trending_reels"];
  ...
  it("keeps the given order for a partial saved list and appends missing keys", () => {
    ...
    expect(out).toEqual(["trending_reels", "stories", "continue_watching"]);
  });
  ...
  it("drops a stale 'friend_activity' entry — retired as a Home module (2026-07-11); an existing viewer's saved preferences row may still contain it", () => {
    const out = normalizeOrder(["stories", "friend_activity", "trending_reels"]);
    expect(out).not.toContain("friend_activity");
    expect(out).toHaveLength(HOME_MODULE_KEYS.length);
  });
  ...
  it("dedupes a repeated valid key instead of returning it twice (a real bug found in review: a duplicate used to survive and inflate the output past HOME_MODULE_KEYS.length)", () => {
    const out = normalizeOrder(["stories", "stories", "trending_reels"]);
    expect(out).toHaveLength(HOME_MODULE_KEYS.length);
    expect(new Set(out).size).toBe(HOME_MODULE_KEYS.length);
    expect(out).toEqual(["stories", "trending_reels", "continue_watching"]);
  });
});
