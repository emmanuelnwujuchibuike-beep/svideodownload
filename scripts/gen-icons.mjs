...
async function makeFlatIcons() {
  ...
  for (const size of [192, 512]) {
    await sharp(await composedIcon(size, 0.7)).toFile(path.join(PUB, `icon-${size}.png`));
...
async function makeMaskable() {
  ...
  await sharp(await composedIcon(512, 0.62)).toFile(path.join(PUB, "icon-maskable-512.png"));
...
async function makeStoreIcon() {
  ...
  await sharp(await composedIcon(1024, 0.7)).toFile(path.join(PUB, "icon-1024.png"));
...
async function makeAppleIcon() {
  ...
  await sharp(await composedIcon(180, 0.7)).toFile(path.join(APP, "apple-icon.png"));
...
async function makeAppleLegacySizes() {
  ...
  for (const size of [152, 167]) {
    await sharp(src).resize(size, size).png().toFile(path.join(PUB, `apple-icon-${size}.png`));
...
async function makeOgIconData() {
  ...
  const trimmed = await sharp(SOURCE).trim().toBuffer();
  const buf = await sharp(trimmed).resize({ width: 480, height: 480, fit: "inside" }).png().toBuffer();
  ...
  console.log(`wrote components/og-icon-data.ts (${(base64.length / 1024).toFixed(0)}KB base64)`);
}
